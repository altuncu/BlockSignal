...
import com.example.altuncu.blocksignal.components.webrtc.WebRtcAnswerDeclineButton;
import com.example.altuncu.blocksignal.components.webrtc.WebRtcCallControls;
import com.example.altuncu.blocksignal.components.webrtc.WebRtcCallScreen;
import com.example.altuncu.blocksignal.crypto.storage.TextSecureIdentityKeyStore;
import com.example.altuncu.blocksignal.events.WebRtcViewModel;
...
public class WebRtcCallActivity extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(com.example.altuncu.blocksignal.R.layout.webrtc_call_activity);
  ...
  private void initializeResources() {
    callScreen = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.callScreen);
  ...
  private void handleAnswerCall() {
    ...
    if (event != null) {
      Permissions.with(this)
                 ...
                 .ifNecessary()
                 .withRationaleDialog(getString(com.example.altuncu.blocksignal.R.string.WebRtcCallActivity_to_answer_the_call_from_s_give_signal_access_to_your_microphone, event.getRecipient().toShortString()),
                 ...
                 .withPermanentDenialDialog(getString(com.example.altuncu.blocksignal.R.string.WebRtcCallActivity_signal_requires_microphone_and_camera_permissions_in_order_to_make_or_receive_calls))
                 .onAllGranted(() -> {
                   callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_answering));
  ...
  private void handleDenyCall() {
    ...
    if (event != null) {
      ...
      callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_ending_call));
      delayedFinish();
    }
  }
  ...
  private void handleOutgoingCall(@NonNull WebRtcViewModel event) {
    callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_dialing));
  }

  private void handleTerminate(@NonNull Recipient recipient /*, int terminationType */) {
    Log.w(TAG, "handleTerminate called");

    callScreen.setActiveCall(recipient, getString(com.example.altuncu.blocksignal.R.string.RedPhone_ending_call));
  ...
  private void handleCallRinging(@NonNull WebRtcViewModel event) {
    callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_ringing));
  }

  private void handleCallBusy(@NonNull WebRtcViewModel event) {
    callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_busy));
  ...
  private void handleCallConnected(@NonNull WebRtcViewModel event) {
    ...
    callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_connected), "");
  }

  private void handleRecipientUnavailable(@NonNull WebRtcViewModel event) {
    callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_recipient_unavailable));
    delayedFinish();
  }

  private void handleServerFailure(@NonNull WebRtcViewModel event) {
    callScreen.setActiveCall(event.getRecipient(), getString(com.example.altuncu.blocksignal.R.string.RedPhone_network_failed));
    delayedFinish();
  }
  ...
  private void handleNoSuchUser(final @NonNull WebRtcViewModel event) {
    ...
    dialog.setTitle(com.example.altuncu.blocksignal.R.string.RedPhone_number_not_registered);
    dialog.setIconAttribute(com.example.altuncu.blocksignal.R.attr.dialog_alert_icon);
    dialog.setMessage(com.example.altuncu.blocksignal.R.string.RedPhone_the_number_you_dialed_does_not_support_secure_voice);
    dialog.setCancelable(true);
    dialog.setPositiveButton(com.example.altuncu.blocksignal.R.string.RedPhone_got_it, new OnClickListener() {
      @Override
