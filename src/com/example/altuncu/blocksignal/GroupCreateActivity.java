...
import com.example.altuncu.blocksignal.components.PushRecipientsPanel.RecipientsPanelChangedListener;
import com.example.altuncu.blocksignal.contacts.ContactsCursorLoader.DisplayMode;
import com.example.altuncu.blocksignal.contacts.RecipientsEditor;
import com.example.altuncu.blocksignal.contacts.avatars.ContactColors;
...
import com.example.altuncu.blocksignal.groups.GroupManager;
import com.example.altuncu.blocksignal.groups.GroupManager.GroupActionResult;
...
import com.example.altuncu.blocksignal.util.SelectedRecipientsAdapter;
import com.example.altuncu.blocksignal.util.SelectedRecipientsAdapter.OnRecipientDeletedListener;
                                            ...
                                            RecipientsPanelChangedListener
{
  ...
  protected void onCreate(Bundle state, boolean ready) {
    setContentView(com.example.altuncu.blocksignal.R.layout.group_create_activity);
  ...
  private void disableSignalGroupViews(int reasonResId) {
    View pushDisabled = findViewById(com.example.altuncu.blocksignal.R.id.push_disabled);
    pushDisabled.setVisibility(View.VISIBLE);
    ((TextView) findViewById(com.example.altuncu.blocksignal.R.id.push_disabled_reason)).setText(reasonResId);
    avatar.setEnabled(false);
    groupName.setEnabled(false);
  }

  private void enableSignalGroupViews() {
    findViewById(com.example.altuncu.blocksignal.R.id.push_disabled).setVisibility(View.GONE);
  ...
  private void updateViewState() {
    if (!TextSecurePreferences.isPushRegistered(this)) {
      disableSignalGroupViews(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_youre_not_registered_for_signal);
      getSupportActionBar().setTitle(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_actionbar_mms_title);
    } else if (getAdapter().hasNonPushMembers()) {
      disableSignalGroupViews(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_contacts_dont_support_push);
      getSupportActionBar().setTitle(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_actionbar_mms_title);
    } else {
      enableSignalGroupViews();
      getSupportActionBar().setTitle(groupToUpdate.isPresent()
                                     ? com.example.altuncu.blocksignal.R.string.GroupCreateActivity_actionbar_edit_title
                                     : com.example.altuncu.blocksignal.R.string.GroupCreateActivity_actionbar_title);
    }
  }
  ...
  private void initializeResources() {
    RecipientsEditor    recipientsEditor = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.recipients_text);
    PushRecipientsPanel recipientsPanel  = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.recipients);
    lv           = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.selected_contacts_list);
    avatar       = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.avatar);
    groupName    = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.group_name);
    creatingText = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.creating_group_text);
    SelectedRecipientsAdapter adapter = new SelectedRecipientsAdapter(this);
    adapter.setOnRecipientDeletedListener(this);
    lv.setAdapter(adapter);
    recipientsEditor.setHint(com.example.altuncu.blocksignal.R.string.recipients_panel__add_members);
    recipientsPanel.setPanelChangeListener(this);
    findViewById(com.example.altuncu.blocksignal.R.id.contacts_button).setOnClickListener(new AddRecipientButtonListener());
    avatar.setImageDrawable(new ResourceContactPhoto(com.example.altuncu.blocksignal.R.drawable.ic_group_white_24dp).asDrawable(this, ContactColors.UNKNOWN_COLOR.toConversationColor(this)));
  ...
  public boolean onPrepareOptionsMenu(Menu menu) {
    ...
    inflater.inflate(com.example.altuncu.blocksignal.R.menu.group_create, menu);
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    switch (item.getItemId()) {
      case android.R.id.home:
        finish();
        return true;
      case com.example.altuncu.blocksignal.R.id.menu_create_group:
  ...
  private void handleGroupCreate() {
    if (getAdapter().getCount() < 1) {
      Log.i(TAG, getString(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_contacts_no_members));
      Toast.makeText(getApplicationContext(), com.example.altuncu.blocksignal.R.string.GroupCreateActivity_contacts_no_members, Toast.LENGTH_SHORT).show();
      return;
    }
    if (isSignalGroup()) {
  ...
  private abstract static class SignalGroupTask extends AsyncTask<Void,Void,Optional<GroupActionResult>> {
    ...
    protected void onPreExecute() {
      activity.findViewById(com.example.altuncu.blocksignal.R.id.group_details_layout).setVisibility(View.GONE);
      activity.findViewById(com.example.altuncu.blocksignal.R.id.creating_group_layout).setVisibility(View.VISIBLE);
      activity.findViewById(com.example.altuncu.blocksignal.R.id.menu_create_group).setVisibility(View.GONE);
        final int titleResId = activity.groupToUpdate.isPresent()
                             ? com.example.altuncu.blocksignal.R.string.GroupCreateActivity_updating_group
                             : com.example.altuncu.blocksignal.R.string.GroupCreateActivity_creating_group;
    ...
    protected void onPostExecute(Optional<GroupActionResult> groupActionResultOptional) {
      ...
      activity.findViewById(com.example.altuncu.blocksignal.R.id.group_details_layout).setVisibility(View.VISIBLE);
      activity.findViewById(com.example.altuncu.blocksignal.R.id.creating_group_layout).setVisibility(View.GONE);
      activity.findViewById(com.example.altuncu.blocksignal.R.id.menu_create_group).setVisibility(View.VISIBLE);
    }
  }
    ...
    protected void onPostExecute(Optional<GroupActionResult> result) {
      if (result.isPresent() && result.get().getThreadId() > -1) {
        ...
        Toast.makeText(activity.getApplicationContext(),
                       com.example.altuncu.blocksignal.R.string.GroupCreateActivity_contacts_invalid_number, Toast.LENGTH_LONG).show();
      }
    }
  }
    ...
    protected List<Result> doInBackground(Recipient... recipients) {
      ...
      for (Recipient recipient : recipients) {
        ...
        if (failIfNotPush && !isPush) {
          results.add(new Result(null, false, activity.getString(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_cannot_add_non_push_to_existing_group,
                                                                 recipient.toShortString())));
        } else if (TextUtils.equals(TextSecurePreferences.getLocalNumber(activity), recipient.getAddress().serialize())) {
          results.add(new Result(null, false, activity.getString(com.example.altuncu.blocksignal.R.string.GroupCreateActivity_youre_already_in_the_group)));
        } else {
  ...
  private static class FillExistingGroupInfoAsyncTask extends ProgressDialogAsyncTask<String,Void,Optional<GroupData>> {
    ...
    public FillExistingGroupInfoAsyncTask(GroupCreateActivity activity) {
      super(activity,
            com.example.altuncu.blocksignal.R.string.GroupCreateActivity_loading_group_details,
            com.example.altuncu.blocksignal.R.string.please_wait);
      this.activity = activity;
    }
