...
import com.example.altuncu.blocksignal.components.MediaView;
import com.example.altuncu.blocksignal.components.viewpager.ExtendedOnPageChangedListener;
...
import com.example.altuncu.blocksignal.database.MediaDatabase.MediaRecord;
import com.example.altuncu.blocksignal.database.loaders.PagingMediaLoader;
...
import com.example.altuncu.blocksignal.util.AttachmentUtil;
...
public class MediaPreviewActivity extends PassphraseRequiredActionBarActivity implements RecipientModifiedListener, LoaderManager.LoaderCallbacks<Pair<Cursor, Integer>> {
  ...
  protected void onCreate(Bundle bundle, boolean ready) {
    this.setTheme(com.example.altuncu.blocksignal.R.style.TextSecure_DarkTheme);
    ...
    setContentView(com.example.altuncu.blocksignal.R.layout.media_preview_activity);
  ...
  private void initializeActionBar() {
    ...
    if (mediaItem != null) {
      ...
      if (mediaItem.date > 0) {
        ...
        relativeTimeSpan = getString(com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_draft);
      }

      if      (mediaItem.outgoing)          getSupportActionBar().setTitle(getString(com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_you));
  ...
  private void initializeViews() {
    mediaPager = findViewById(com.example.altuncu.blocksignal.R.id.media_pager);
  ...
  private void initializeMedia() {
    if (!isContentTypeSupported(initialMediaType)) {
      ...
      Toast.makeText(getApplicationContext(), com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_unssuported_media_type, Toast.LENGTH_LONG).show();
      finish();
    }
    ...
    if (mediaItem != null) {
      SaveAttachmentTask.showWarningDialog(this, (dialogInterface, i) -> {
        Permissions.with(this)
                   ...
                   .ifNecessary()
                   .withPermanentDenialDialog(getString(com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_signal_needs_the_storage_permission_in_order_to_write_to_external_storage_but_it_has_been_permanently_denied))
                   .onAnyDenied(() -> Toast.makeText(this, com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_unable_to_write_to_external_storage_without_permission, Toast.LENGTH_LONG).show())
                   .onAllGranted(() -> {
  ...
  private void deleteMedia() {
    ...
    builder.setTitle(com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_media_delete_confirmation_title);
    builder.setMessage(com.example.altuncu.blocksignal.R.string.MediaPreviewActivity_media_delete_confirmation_message);
    builder.setCancelable(true);

    builder.setPositiveButton(com.example.altuncu.blocksignal.R.string.delete, (dialogInterface, which) -> {
  ...
  public boolean onPrepareOptionsMenu(Menu menu) {
    ...
    inflater.inflate(com.example.altuncu.blocksignal.R.menu.media_preview, menu);

    if (!isMediaInDb()) {
      menu.findItem(com.example.altuncu.blocksignal.R.id.media_preview__overview).setVisible(false);
      menu.findItem(com.example.altuncu.blocksignal.R.id.delete).setVisible(false);
    }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    switch (item.getItemId()) {
      case com.example.altuncu.blocksignal.R.id.media_preview__overview: showOverview(); return true;
      case com.example.altuncu.blocksignal.R.id.media_preview__forward:  forward();      return true;
      case com.example.altuncu.blocksignal.R.id.save:                    saveToDisk();   return true;
      case com.example.altuncu.blocksignal.R.id.delete:                  deleteMedia();  return true;
  ...
  private static class SingleItemPagerAdapter extends PagerAdapter implements MediaItemAdapter {
    ...
    public @NonNull Object instantiateItem(@NonNull ViewGroup container, int position) {
      View      itemView  = inflater.inflate(com.example.altuncu.blocksignal.R.layout.media_view_page, container, false);
      MediaView mediaView = itemView.findViewById(com.example.altuncu.blocksignal.R.id.media_view);
    ...
    public void destroyItem(@NonNull ViewGroup container, int position, @NonNull Object object) {
      MediaView mediaView = ((FrameLayout)object).findViewById(com.example.altuncu.blocksignal.R.id.media_view);
      mediaView.cleanup();
  ...
  private static class CursorPagerAdapter extends PagerAdapter implements MediaItemAdapter {
    ...
    @Override
    public @NonNull Object instantiateItem(@NonNull ViewGroup container, int position) {
      View      itemView       = LayoutInflater.from(context).inflate(com.example.altuncu.blocksignal.R.layout.media_view_page, container, false);
      MediaView mediaView      = itemView.findViewById(com.example.altuncu.blocksignal.R.id.media_view);
