...
import com.example.altuncu.blocksignal.database.RecipientDatabase.VibrateState;
import com.example.altuncu.blocksignal.database.loaders.ThreadMediaLoader;
...
import com.example.altuncu.blocksignal.preferences.CorrectedPreferenceFragment;
import com.example.altuncu.blocksignal.preferences.widgets.ColorPickerPreference;
import com.example.altuncu.blocksignal.preferences.widgets.ContactPreference;
...
public class RecipientPreferenceActivity extends PassphraseRequiredActionBarActivity implements RecipientModifiedListener, LoaderManager.LoaderCallbacks<Cursor>
{
  ...
  public void onCreate(Bundle instanceState, boolean ready) {
    setContentView(com.example.altuncu.blocksignal.R.layout.recipient_preference_activity);
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    Fragment fragment = getSupportFragmentManager().findFragmentById(com.example.altuncu.blocksignal.R.id.preference_fragment);
  ...
  private void initializeToolbar() {
    this.toolbarLayout        = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.collapsing_toolbar);
    this.avatar               = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.avatar);
    this.threadPhotoRailView  = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.recent_photos);
    this.threadPhotoRailLabel = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.rail_label);

    this.toolbarLayout.setExpandedTitleColor(getResources().getColor(com.example.altuncu.blocksignal.R.color.white));
    this.toolbarLayout.setCollapsedTitleTextColor(getResources().getColor(com.example.altuncu.blocksignal.R.color.white));
    ...
    Toolbar toolbar = ViewUtil.findById(this, com.example.altuncu.blocksignal.R.id.toolbar);
  ...
  public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    ...
    initFragment(com.example.altuncu.blocksignal.R.id.preference_fragment, new RecipientPreferenceFragment(), null, bundle);
  }
      ...
      implements RecipientModifiedListener
  {
    ...
    public void onCreatePreferences(@Nullable Bundle savedInstanceState, String rootKey) {
      ...
      addPreferencesFromResource(com.example.altuncu.blocksignal.R.xml.recipient_preferences);
    }
    ...
    private void setSummaries(Recipient recipient) {
      ...
      if (recipient.isGroupRecipient()) {
        ...
        if (recipient.isBlocked()) blockPreference.setTitle(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_unblock);
        else                       blockPreference.setTitle(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_block);
        ...
        IdentityUtil.getRemoteIdentityKey(getActivity(), recipient).addListener(new ListenableFuture.Listener<Optional<IdentityRecord>>() {
          ...
          public void onSuccess(Optional<IdentityRecord> result) {
            if (result.isPresent()) {
              ...
              if (identityPreference != null) identityPreference.setSummary(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_available_once_a_message_has_been_sent_or_received);
    ...
    private @NonNull String getRingtoneSummary(@NonNull Context context, @Nullable Uri ringtone) {
      if (ringtone == null) {
        return context.getString(com.example.altuncu.blocksignal.R.string.preferences__default);
      } else if (ringtone.toString().isEmpty()) {
        return context.getString(com.example.altuncu.blocksignal.R.string.preferences__silent);
      } else {
    ...
    private @NonNull Pair<String, Integer> getVibrateSummary(@NonNull Context context, @NonNull VibrateState vibrateState) {
      if (vibrateState == VibrateState.DEFAULT) {
        return new Pair<>(context.getString(com.example.altuncu.blocksignal.R.string.preferences__default), 0);
      } else if (vibrateState == VibrateState.ENABLED) {
        return new Pair<>(context.getString(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_enabled), 1);
      } else {
        return new Pair<>(context.getString(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_disabled), 2);
      }
    }
    ...
    private class BlockClickedListener implements Preference.OnPreferenceClickListener {
      ...
      private void handleBlock() {
        new AlertDialog.Builder(getActivity())
            .setTitle(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_block_this_contact_question)
            .setMessage(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_you_will_no_longer_receive_messages_and_calls_from_this_contact)
            .setCancelable(true)
            .setNegativeButton(android.R.string.cancel, null)
            .setPositiveButton(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_block, new DialogInterface.OnClickListener() {
              @Override
      ...
      private void handleUnblock() {
        new AlertDialog.Builder(getActivity())
            .setTitle(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_unblock_this_contact_question)
            .setMessage(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_you_will_once_again_be_able_to_receive_messages_and_calls_from_this_contact)
            .setCancelable(true)
            .setNegativeButton(android.R.string.cancel, null)
            .setPositiveButton(com.example.altuncu.blocksignal.R.string.RecipientPreferenceActivity_unblock, new DialogInterface.OnClickListener() {
              @Override
              public void onClick(DialogInterface dialog, int which) {
    ...
    private class AboutNumberClickedListener implements ContactPreference.Listener {
      ...
      public void onSecureCallClicked() {
        Permissions.with(RecipientPreferenceFragment.this)
                   ...
                   .ifNecessary()
                   .withRationaleDialog(getString(com.example.altuncu.blocksignal.R.string.ConversationActivity_to_call_s_signal_needs_access_to_your_microphone_and_camera, recipient.toShortString()),
                                        com.example.altuncu.blocksignal.R.drawable.ic_mic_white_48dp, com.example.altuncu.blocksignal.R.drawable.ic_videocam_white_48dp)
                   .withPermanentDenialDialog(getString(com.example.altuncu.blocksignal.R.string.ConversationActivity_signal_needs_the_microphone_and_camera_permissions_in_order_to_call_s, recipient.toShortString()))
                   .onAllGranted(() -> {
